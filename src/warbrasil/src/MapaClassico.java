...
public class MapaClassico extends Mapa {
    ...
    public void verificar_jogador(String cor_do_jogador_perdedor){
        if(this.getJogador(cor_do_jogador_perdedor).getQtd_estados()==0){
            this.players.remove(this.getEstado(cor_do_jogador_perdedor));
        }
    }
    @Override
            ...
            for (int j = 0; j< x+y; j++) {
                ...
                for(int i = 0; i<qtdPlayers; i++) {
                    if(!nums.isEmpty()) {
                        ...
                        Exercito aux = exercitos.get(players.pollFirst());
                        Exercito ec = new Exercito(aux.getQTD(), aux.getCOR());//Cria um objeto cópia
                        temp.setExercito(ec);
...
//            }
    ...
    public Jogador getJogador(String cor) {
